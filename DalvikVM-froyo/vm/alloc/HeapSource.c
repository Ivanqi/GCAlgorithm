...
struct HeapSource {
     ...
     * 
     * 数组heaps的元素数量是用宏HEAP_SOURCE_MAX_HEAP_COUNT固定的
     */
...
/**
 * 取出VM HeapSource的heaps的第0个元素
 * 程序只使用VM HeapSource 开头的 VM Heap，而这部分进行的正是这项操作。
 * 也就是说从当前活动的VM Heap分配对象
 */
...
countAllocation(Heap *heap, const void *ptr, bool isObj)
{
    ...
    if (isObj) {
        ...
        /**
         * 负责把对应已分配对象的对象位图表格内的位重写成1(正在分配的标记)
         * 也就是说，每当我们分配对象时，对象肯定会被标记到对象位图
         */
...
/**
 * 生成VM Heap
 * 把VM HeapSource的hs、VM Heap的实体msp以及VM Heap空间的最大大小mspAbsoluteMaxSize作为参数接收
 */
static bool
addNewHeap(HeapSource *hs, mspace msp, size_t mspAbsoluteMaxSize)
{
    ...
    /**
     * 检查参数msp是否为NULL
     * 如果不为NULL，就把msp和mspAbsoluteMaxSize设定为结构体Heap
     * 如果msp为NULL，生成新的VM Heap
     */
    if (msp != NULL) {
        heap.msp = msp;
        ...
        /**
         * oldHeapOverhead()函数对现在VMHeapSource中的VM Heap返回实际被分配的内存空间大小
         */
    ...
    /**
     * 生成位图
     * ALIGN_DOWN_TO_PAGE_SIZE()，这个宏把获取的地址以4K字节去尾对齐，也就是说，如果把"3K字节"传递给这个宏，就会返回"0"
     * 
     * 参数说明
     *  未初始化的HeapBitmap
     *  将VMHeap的开头地址(heap.msp)以4K字节对齐的值
     *  最大VM Heap大小(heap.absoluteMaxSize)
     *  "objects"(字符串)
     */
    ...
    if (hs->numHeaps > 0) {
        /**
         * 给heaps[0].msp(现在正在使用的VM Heap)加上一个限制条件，使其不能进行新的分配
         */
        mspace msp = hs->heaps[0].msp;
        /**
         * mspace_footprint() 函数是用 dlmalloc 定义的
         * 把dlmalloc生成的mspace的指针传递给这个函数，就会返回msapce实际使用的(正在分配的)字节数
         * 函数名中的footprint的意思是"使用的内存量"
         * 
         * mspace_set_max_allowed_footprint() 函数是对内存空间设置制约条件的函数，它规定了"只能分配到这个字节数为止"，这个函数也是用dlmalloc定义
         */
     ...
     * 
     * 把新的VM Heap 插入到VM HeapSource的开头
     */
 ...
 * 
 * VM HeapSource是用dvmHeapSourceStartup()函数生成的
 * absoluteMaxSize: 是能将内存分配给VM Heap的 "最大VM Heap大小"
 */
GcHeap *
dvmHeapSourceStartup(size_t startSize, size_t absoluteMaxSize)
{
    ...
    /**
     * softLimit初始化
     * softLimit是由DalvikVM决定的，它是能分配到VM Heap空间的软件上的界限
     * 已知这里已经设定了INT_MAX,这个宏里定义了int的最大值(2147 483 647)
     */
    hs->softLimit = INT_MAX;    // no soft limit at first
    hs->numHeaps = 0;
    /**
     * 全局变量gDvm.这个全局变量持有DalvikVM相关的各种设定值
     */
    ...
    /**
     * 调用countAllocation()，把之前分配的gcHeap等的大小加到bytesAllocated(VM Heap使用的字节数)
     */
    countAllocation(hs2heap(hs), gcHeap, false);
    countAllocation(hs2heap(hs), hs, false);

    // 设定这次生成的VM HeapSource的指针
    gHs = hs;
    return gcHeap;
 ...
 * dvmHeapSourceAlloc()函数就是最终把对象分配给DalvikVM的VM Heap空间的函数
 */
void *
dvmHeapSourceAlloc(size_t n)
{
    // 将全局变量gHs的VM HeapSource存入hs
    HeapSource *hs = gHs;
    Heap *heap;
    void *ptr;
    ...
    /**
     * 把当前分配到VM Heap的字节数和这次分配的字节数相加，确认其结果小于等于softLimit(DalvikVM指定的软件上的分配界限)
     */
...
// TODO: zero out large objects using madvise
        /**
         * 从VM Heap分配内存空间
         * mspace_calloc()函数是用dlmalloc定义的函数。虽然参数多多少少有些不同，不过其功能和C语言的calloc()函数相同
         * 都是分配n个指定的大小，将内存清空为0后返回其开头的指针
         * 在dlmalloc中，按惯例会把msp(VM Heap)传递给第1个参数
         * 
         * 从dlmalloc分配的指针是以8字节对齐的。也就是说，对象指针的地址肯定是8的倍数
         */
