        ...
        GcMarkContext *ctx)
{
    if (clazz->refOffsets != CLASS_WALK_SUPER) {
        ...
        while (clazz != NULL) {
             ...
             * 
             * 把对象内的实例字段取出来，并传递给了markObject()函数
             * markObject()函数负责设置从参数接收到的对象的标记位
             */
            f = clazz->ifields;
...
processMarkStack(GcMarkContext *ctx)
{
     ...
     */
    // 把ULONG_MAX设定给finger.这个宏里定义了unsigned long 型的最大正数
    ctx->finger = (void *)ULONG_MAX;
    /**
     * 把堆积在标记栈里的对象进行标记
     * scanObject()函数不会递归地标记对象成员
     * 但因为finger里存着unsigned long 型的最大正数，所以新的对象指针会不断被追加到标记栈中
     * 只要在追加过程结束之前一直调用scanObject()，就不会漏掉任何一个该标记的对象
     */
...
/**
 * 首先从dvmHeapBitmapXorWalk()函数设定了标记对象指针
 * 然后用scanObject()函数把对象指针从这些标记对象指针的缓冲区中一个个取出来
 * scanBitmapCallback()函数只负责调用这个scanObject()函数
*/
static bool
scanBitmapCallback(size_t numPtrs, void **ptrs, const void *finger, void *arg)
{
    ...
    for (i = 0; i < numPtrs; i++) {
         ...
         */
        /**
         * 宏chunk2ptr()负责向下偏移地址，地址偏移就是用于debug的头(header)的量
         */
...
#endif
    /**
     * dvmHeapBitmapWalkList()函数按照VM Heap地址由低到高的顺序
     * 把与VM Heap对应的所有标记位图交给dvmHeapBitmapWalk()函数
     */
