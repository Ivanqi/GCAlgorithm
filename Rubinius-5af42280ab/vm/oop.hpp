...
/**
 * 给C++的int设置标志位，并转换成Fixnum
 * 在这里向左偏移了1位，跟1做了一下OR
 */
#define APPLY_FIXNUM_TAG(v) ((Object*)(((intptr_t)(v) << TAG_FIXNUM_SHIFT) | TAG_FIXNUM))
/**
 * 如果对设置了标签的数值(Fixnum)执行加法运算，就会返回跟所求结果不同的答案
 * 因此在进行加减运算等操作的时候，要先拿到数值(Fixnum)内的标签位再执行操作
 */
...
#define SIZE_OF_BODY(obj)               (obj->num_fields() * SIZE_OF_OBJECT)
  ...
  class ObjectHeader {
    union {
      struct {
        object_type     obj_type_   : 8;  // 对象的类别ID
        gc_zone         zone        : 2;  // 所属空间
        unsigned int    age         : 4;  // 年龄

        unsigned int Forwarded              : 1;  // 是否forwarding指针
        unsigned int Remember               : 1;  // 是否已被记录在remembered_set中
        unsigned int Marked                 : 2;  // 用于GC的标记
        unsigned int RequiresCleanup        : 1;  // 释放对象时是否调用cleanup()

        unsigned int RefsAreWeak            : 1;  // 是否为弱引用对象

        unsigned int InImmix                : 1;  // 是否为ImmixGC的对象
        unsigned int Pinned                 : 1;  // 不能用ImmixGC移动
      };
