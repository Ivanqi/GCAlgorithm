...
/**
 * 终结器
 * 终结器指的是与对象的释放处理挂钩，进行某些处理的功能
 * 列表和字段等内置数据类型的对象基本上是不能设置终结器的，能定义终结器的只有用户创建的类
 */
static void
slot_tp_del(PyObject *self)
{
	...
	/* Execute __del__ method, if any.如果有__del__就执行它 */
	...
	assert(self->ob_refcnt > 0);
	if (--self->ob_refcnt == 0)
		return;	/* this is the normal path out. 退出函数 */
